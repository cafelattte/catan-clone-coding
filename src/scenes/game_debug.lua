-- src/scenes/game_debug.lua
-- 디버그/테스트용 게임 씬 (기존 main.lua의 테스트 데이터 포함)
-- Story 7-3 백업 파일
-- 사용법: main.lua에서 menu 대신 이 파일을 초기 씬으로 설정
...
function game_debug:enter(previous) -- luacheck: ignore previous
  -- 보드 생성
...
function game_debug:leave()
  -- 정리
end

function game_debug:update(dt) -- luacheck: ignore dt
  -- 업데이트 로직 (나중에 구현)
end
...
function game_debug:draw()
  ...
  love.graphics.print("[DEBUG] Settlus of Catan - FPS: " .. love.timer.getFPS(), 10, screenHeight - 20)
end

-- 키보드 입력 처리 - 선택 모드 토글 (Task 5.1)
function game_debug:keypressed(key)
...
function game_debug:mousemoved(x, y)
  if selectionMode == "none" then
    hoverVertex = nil
    hoverEdge = nil
    return
...
function game_debug:mousepressed(x, y, button)
